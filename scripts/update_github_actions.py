...
def run_gh(command: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(  # noqa: S603  # argv starts with the resolved trusted gh executable
...
def update_github_actions(
    *,
    ...
    updated here. That pin is a deliberate contributor floor (e.g. >=0.11.16)
    and must not auto-track the latest uv release. Auto-tracking caused the
    dep-refresh job to self-poison: it wrote the latest uv as an exact ==
    constraint, which the job's own older pinned uv then violated.
    """
